...
public class CompensateInternalLatencyTest extends BaseHttpScenarioTest {
   ...
   // delaying subsequent session starts. With compensateInternalLatency enabled,
   // startTimestampNanos comes from the rate generator's fire time — unaffected
   // by the delay. Without it, System.nanoTime() at the delayed Request.start()
   // would produce inflated diffs.
   ...
   public void testEnabledStartTimestampsMatchIntendedFireTimes() {
      long[] captured = runWithCompensation(true);
   ...
   public void testDisabledStartTimestampsInflatedByDelay() {
      ...
      // Without compensateInternalLatency, Request.start() uses System.nanoTime().
      // The sleep blocks the event loop beyond the intended spacing, so at least
      // one consecutive diff must be >= the sleep duration.
      long maxDiff = 0;
      for (int i = 1; i < captured.length; i++) {
         long diff = captured[i] - captured[i - 1];
         if (diff > maxDiff) {
      ...
      assertThat(maxDiff)
            .describedAs("Without compensation, max diff should be inflated by the sleep")
